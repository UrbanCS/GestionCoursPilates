...
Le taux de taxes est unique pour le studio. Un Super User peut le modifier dans **Paramètres → Général → Taxes** depuis l’administration Joomla ou le portail frontal. La valeur initiale est `14,975 %`; elle s’applique aux nouvelles commandes de cours et de forfaits. Les prix du catalogue sont présentés avant taxes; le sous-total, les taxes et le total exacts apparaissent lorsque le client ouvre le formulaire de paiement. Les commandes déjà créées conservent les montants enregistrés au moment de leur création.
