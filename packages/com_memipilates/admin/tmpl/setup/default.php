...
<div class="container-fluid memi-admin-setup">
    ...
    <div class="row row-cols-2 row-cols-md-4 g-3 mb-4">
        <div class="col"><div class="card h-100"><div class="card-body"><span class="text-muted small"><?= $escape($label('COM_MEMIPILATES_SETUP_LOCATIONS', 'Studio')); ?></span><div class="fs-3 fw-bold"><?= count($this->locations); ?></div></div></div></div>
    ...
    <section class="card mb-4">
        <div class="card-header"><h2 class="h5 mb-0">1. <?= $escape($label('COM_MEMIPILATES_SETUP_LOCATION', 'Studio')); ?></h2></div>
        <div class="card-body">
            <form action="<?= $postUrl; ?>" method="post" class="row g-3">
                ...
                <div class="col-12"><button class="btn btn-primary" type="submit"><?= $escape($label('COM_MEMIPILATES_SETUP_SAVE_LOCATION', 'Save studio')); ?></button></div>
            </form>
        </div>
    </section>
    ...
    <section class="card mb-4">
        ...
        <div class="card-body">
            <?php if ($this->locations === []) : ?><p class="text-muted mb-0"><?= $escape($label('COM_MEMIPILATES_SETUP_PREREQUISITE_LOCATION', 'Configure the studio first.')); ?></p><?php else : ?>
            <form action="<?= $postUrl; ?>" method="post" class="row g-3">
                <?= $entityFields('room'); ?>
                <input type="hidden" name="location_id" value="<?= (int) $this->locations[0]['id']; ?>">
                <div class="col-12 col-md-10"><label class="form-label" for="room-title"><?= $escape(Text::_('JGLOBAL_TITLE')); ?></label><input class="form-control" id="room-title" name="title" maxlength="255" required></div>
    ...
    <section class="card mb-4">
        ...
        <div class="card-body">
            <form action="<?= $postUrl; ?>" method="post" class="row g-3">
                ...
                <div class="col-12 col-md-4"><label class="form-label" for="instructor-user"><?= $escape($label('COM_MEMIPILATES_INSTRUCTOR_JOOMLA_USER_ID', 'Linked Joomla account (ID)')); ?></label><input class="form-control" id="instructor-user" name="user_id" type="number" min="1"><small class="form-text"><?= $escape($label('COM_MEMIPILATES_INSTRUCTOR_JOOMLA_USER_ID_HELP', 'Optional. Without this link, the instructor account cannot access any assigned sessions.')); ?></small></div>
