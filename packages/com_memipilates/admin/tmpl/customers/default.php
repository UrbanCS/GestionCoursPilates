...
$columnCount = 2 + ($this->canViewClientDetails ? 6 : 0) + ($this->canManageQr ? 1 : 0);
?>
<div class="container-fluid memi-admin-customers">
    ...
    <div class="table-responsive"><table class="table table-striped table-hover align-middle"><thead><tr><th><?= $escape(Text::_('JGRID_HEADING_ID')); ?></th><th><?= $escape($this->label('COM_MEMIPILATES_ADMIN_CUSTOMER', 'Customer')); ?></th><?php if ($this->canViewClientDetails) : ?><th><?= $escape($this->label('COM_MEMIPILATES_ADMIN_PHONE', 'Phone')); ?></th><th><?= $escape($this->label('COM_MEMIPILATES_ADMIN_CREDIT_BALANCE', 'Credit balance')); ?></th><th><?= $escape($this->label('COM_MEMIPILATES_ADMIN_POINT_BALANCE', 'Point balance')); ?></th><th><?= $escape($this->label('COM_MEMIPILATES_ADMIN_ACTIVE_PACKAGES', 'Active packages')); ?></th><th><?= $escape($this->label('COM_MEMIPILATES_ADMIN_JOINED_AT', 'Joined')); ?></th><th><?= $escape($this->label('COM_MEMIPILATES_ADMIN_ACCOUNT_STATUS', 'Account status')); ?></th><?php endif; ?><?php if ($this->canManageQr) : ?><th><?= $escape($this->label('COM_MEMIPILATES_ADMIN_QR_CODE', 'QR code')); ?></th><?php endif; ?></tr></thead><tbody>
        <?php foreach ($this->items as $item) : ?>
            <tr><td><?= (int) $item['user_id']; ?></td><td><strong><?= $escape($item['name'] ?? ''); ?></strong><?php if ($this->canViewClientDetails) : ?><br><small><?= $escape($item['email'] ?? ''); ?></small><?php endif; ?></td><?php if ($this->canViewClientDetails) : ?><td><?= $escape($item['phone'] ?? ''); ?></td><td><?= (int) ($item['credit_balance'] ?? 0); ?></td><td><?= (int) ($item['point_balance'] ?? 0); ?></td><td><?= (int) ($item['active_packages'] ?? 0); ?></td><td><?= $escape($this->formatDate((string) ($item['joined_at'] ?? ''))); ?></td><td><?= $escape((int) ($item['block'] ?? 0) === 1 ? $this->label('COM_MEMIPILATES_ADMIN_BLOCKED', 'Blocked') : $this->label('COM_MEMIPILATES_ADMIN_ACTIVE', 'Active')); ?></td><?php endif; ?><?php if ($this->canManageQr) : ?><td><span class="badge <?= !empty($item['qr_token_id']) ? 'bg-success' : 'bg-secondary'; ?>"><?= $escape(!empty($item['qr_token_id']) ? $this->label('COM_MEMIPILATES_ADMIN_QR_ACTIVE', 'Active') : $this->label('COM_MEMIPILATES_ADMIN_QR_NONE', 'None')); ?></span><div class="d-flex flex-wrap gap-1 mt-2"><form action="<?= $filterUrl; ?>" method="post"><input type="hidden" name="option" value="com_memipilates"><input type="hidden" name="task" value="management.regenerateQr"><input type="hidden" name="user_id" value="<?= (int) $item['user_id']; ?>"><input type="hidden" name="idempotency_key" value="<?= $escape(bin2hex(random_bytes(16))); ?>"><input type="hidden" name="<?= $token; ?>" value="1"><button class="btn btn-sm btn-outline-primary" type="submit"><?= $escape($this->label('COM_MEMIPILATES_ADMIN_QR_REGENERATE', 'Regenerate')); ?></button></form><?php if (!empty($item['qr_token_id'])) : ?><form action="<?= $filterUrl; ?>" method="post"><input type="hidden" name="option" value="com_memipilates"><input type="hidden" name="task" value="management.revokeQr"><input type="hidden" name="user_id" value="<?= (int) $item['user_id']; ?>"><input type="hidden" name="<?= $token; ?>" value="1"><button class="btn btn-sm btn-outline-danger" type="submit"><?= $escape($this->label('COM_MEMIPILATES_ADMIN_QR_REVOKE', 'Revoke')); ?></button></form><?php endif; ?></div></td><?php endif; ?></tr>
        <?php endforeach; ?>
        <?php if ($this->items === []) : ?><tr><td colspan="<?= $columnCount; ?>" class="text-muted"><?= $escape($this->label('COM_MEMIPILATES_ADMIN_NO_RESULTS', 'No records found.')); ?></td></tr><?php endif; ?>
    </tbody></table></div>
