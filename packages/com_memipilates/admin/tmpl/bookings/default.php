...
<div class="container-fluid memi-admin-bookings">
    ...
    <?php if ($this->canManualBooking) : ?>
        <section class="card mb-4" aria-labelledby="memi-manual-booking">
            <div class="card-header"><h2 class="h5 mb-0" id="memi-manual-booking"><?= $escape($this->label('COM_MEMIPILATES_ADMIN_MANUAL_BOOKING', 'Inscrire un client')); ?></h2></div>
            <div class="card-body">
                <p class="text-muted mb-3"><?= $escape($this->label('COM_MEMIPILATES_ADMIN_MANUAL_BOOKING_HELP', 'Inscrit un client existant avec un crédit disponible ou à titre gratuit. Une réservation gratuite ne débite aucun crédit.')); ?></p>
                <form action="<?= $filterUrl; ?>" method="post" class="row g-3">
                    <input type="hidden" name="option" value="com_memipilates"><input type="hidden" name="task" value="management.reserveClient"><input type="hidden" name="<?= $token; ?>" value="1">
                    <div class="col-12 col-lg-5"><label class="form-label" for="memi-manual-client"><?= $escape($this->label('COM_MEMIPILATES_ADMIN_CUSTOMER', 'Client')); ?></label><select class="form-select" id="memi-manual-client" name="user_id" required><option value=""><?= $escape($this->label('COM_MEMIPILATES_ADMIN_SELECT', 'Select')); ?></option><?php foreach ($this->manualClients as $client) : ?><option value="<?= (int) $client['id']; ?>"><?= $escape(($client['name'] ?? '') . ($this->canViewContact && ($client['email'] ?? '') !== '' ? ' — ' . $client['email'] : '')); ?></option><?php endforeach; ?></select></div>
                    <div class="col-12 col-lg-5"><label class="form-label" for="memi-manual-session"><?= $escape($this->label('COM_MEMIPILATES_ADMIN_SESSION', 'Séance')); ?></label><select class="form-select" id="memi-manual-session" name="session_id" required><option value=""><?= $escape($this->label('COM_MEMIPILATES_ADMIN_SELECT', 'Select')); ?></option><?php foreach ($this->manualSessions as $session) : ?><option value="<?= (int) $session['id']; ?>"><?= $escape(($session['course_title'] ?? '') . ' — ' . $this->formatDate((string) ($session['starts_at'] ?? '')) . ' (' . (int) ($session['available_places'] ?? 0) . ' places, ' . (int) ($session['credits_required'] ?? 0) . ' crédits)'); ?></option><?php endforeach; ?></select></div>
                    <div class="col-12 col-lg-2"><label class="form-label" for="memi-manual-mode"><?= $escape($this->label('COM_MEMIPILATES_ADMIN_BOOKING_MODE', 'Mode')); ?></label><select class="form-select" id="memi-manual-mode" name="booking_mode"><option value="credit"><?= $escape($this->label('COM_MEMIPILATES_ADMIN_USE_CREDIT', 'Utiliser un crédit')); ?></option><option value="complimentary"><?= $escape($this->label('COM_MEMIPILATES_ADMIN_COMPLIMENTARY', 'Gratuit')); ?></option></select></div>
                    <div class="col-12 col-lg-10"><label class="form-label" for="memi-manual-note"><?= $escape($this->label('COM_MEMIPILATES_ADMIN_NOTE', 'Note interne')); ?></label><input class="form-control" id="memi-manual-note" name="note" maxlength="500"></div>
                    <div class="col-12 col-lg-2 d-flex align-items-end"><button class="btn btn-primary w-100" type="submit"<?= $this->manualClients === [] || $this->manualSessions === [] ? ' disabled' : ''; ?>><?= $escape($this->label('COM_MEMIPILATES_ADMIN_ACTION_BOOK', 'Inscrire')); ?></button></div>
                </form>
            </div>
        </section>
    <?php endif; ?>
