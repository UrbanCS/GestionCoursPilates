        ...
        <section class="card mb-4" aria-labelledby="memi-manual-booking">
            ...
            <div class="card-body">
                ...
                <form action="<?= $filterUrl; ?>" method="post" class="row g-3">
                    ...
                    <div class="col-12 col-lg-5"><label class="form-label" for="memi-manual-client"><?= $escape($this->label('COM_MEMIPILATES_ADMIN_CUSTOMER', 'Client')); ?></label><select class="form-select" id="memi-manual-client" name="user_id" required><option value=""><?= $escape(Text::_('JOPTION_SELECT')); ?></option><?php foreach ($this->manualClients as $client) : ?><option value="<?= (int) $client['id']; ?>"><?= $escape(($client['name'] ?? '') . ($this->canViewContact && ($client['email'] ?? '') !== '' ? ' — ' . $client['email'] : '')); ?></option><?php endforeach; ?></select></div>
                    <div class="col-12 col-lg-5"><label class="form-label" for="memi-manual-session"><?= $escape($this->label('COM_MEMIPILATES_ADMIN_SESSION', 'Séance')); ?></label><select class="form-select" id="memi-manual-session" name="session_id" required><option value=""><?= $escape(Text::_('JOPTION_SELECT')); ?></option><?php foreach ($this->manualSessions as $session) : ?><option value="<?= (int) $session['id']; ?>"><?= $escape(($session['course_title'] ?? '') . ' — ' . $this->formatDate((string) ($session['starts_at'] ?? '')) . ' (' . (int) ($session['available_places'] ?? 0) . ' places, ' . (int) ($session['credits_required'] ?? 0) . ' crédits)'); ?></option><?php endforeach; ?></select></div>
